...
// ==========================
// ✅ FORMATADOR DE ATA (anti-poluição)
// ==========================
function escapeRegExp(str) {
  return String(str).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

function autoBullets(markdown) {
  ...
  for (const line of lines) {
    ...
    // se termina com ":" assume que virão itens em seguida
...
function formatAtaMarkdown(raw, { titulo, dataBR } = {}) {
  ...
  // remove "Claro! Aqui está..." e variações (quando for abertura genérica)
  t = t
    .replace(/^claro!?[^\n]*$/gim, (m) =>
      /aqui est[aá] a ata/i.test(m) ? "" : m
    )
    .trim();

  // Se não tiver H1, cria um header limpo usando dados da reunião
  const hasH1 = /^#\s+/m.test(t);
  const safeTitulo = (titulo || "").trim();
  const safeData = (dataBR || "").trim();

  if (!hasH1 && safeTitulo) {
    t = `# ${safeTitulo}\n\n${safeData ? `**Data:** ${safeData}\n\n` : ""}${t}`;
  }

  // Normaliza padrões "Data: 04/02/2026"
  t = t.replace(/^\s*data\s*:\s*(.+)$/gim, (_, v) => `**Data:** ${String(v).trim()}`);

  // Seções numeradas -> H2
  // "1. Resumo" / "1) Resumo" -> "## 1. Resumo"
  t = t.replace(/^\s*(\d+)\s*[\.\)]\s*(.+)\s*$/gm, (_, n, name) => `## ${n}. ${String(name).trim()}`);

  // Negrito em percentuais
  t = t.replace(/(\b\d{1,3})\s*%/g, "**$1%**");

  // Negrito em labels (somente quando estão no começo da linha)
  const labels = [
    "Objetivo",
    "Participantes",
    "Indicadores",
    "Cumprimento de partida",
    "Pontualidade",
    "Causas",
    "Perdas",
    "Ocorrências",
  ...
  for (const lb of labels) {
    const re = new RegExp(`^\\s*(${escapeRegExp(lb)})\\s*[:\\-]\\s*`, "gmi");
    t = t.replace(re, `**$1:** `);
  }

  // Melhora legibilidade: força linhas “soltas” em bullets após ":" (indicadores/causas/etc)
  t = autoBullets(t);

  // Quebras mínimas para evitar “paredão” (sem inventar conteúdo)
  t = t
    .replace(/\s+(Foi mencionado que)/g, "\n\n$1")
    .replace(/\s+(Foi citado que)/g, "\n\n$1")
    .replace(/\s+(No cumprimento de partida)/gi, "\n\n$1")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
...
export default function CentralAtas() {
  ...
  useEffect(() => {
    if (selectedAta) {
      ...
      // ✅ aplica formatação para deixar a ATA “padronizada” no editor e na leitura
    ...
    try {
      ...
      if (!error && data) {
        ...
        if (
          ...
          // ✅ se veio texto novo, normaliza também
          if (data.pauta) {
            const dataBR = data.data_hora
              ? new Date(data.data_hora).toLocaleDateString("pt-BR")
              : "";
            setEditedPauta(
  ...
  const handleSolicitarVideo = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        alert(
          "Salvo na fila! O GitHub iniciará no próximo ciclo (ou tente novamente para forçar o início)."
        );
      } else {
      ...
      setAtas((prev) =>
        prev.map((a) => (a.id === selectedAta.id ? { ...a, gravacao_status: "PENDENTE" } : a))
      );
  ...
  const handleSaveAta = async () => {
    ...
    if (!error) {
      ...
      setAtas((prev) =>
        prev.map((a) =>
          a.id === selectedAta.id ? { ...a, pauta: editedPauta, ata_manual: ataManual } : a
        )
      );
    ...
    try {
      ...
      reader.onloadend = async () => {
        try {
          ...
          // ✅ AQUI: padroniza para não ficar poluído
          const textoFormatado = formatAtaMarkdown(textoBruto, {
            titulo,
            dataBR,
          });
          ...
          setAtas((prev) =>
            prev.map((a) =>
              a.id === selectedAta.id ? { ...a, pauta: textoFormatado, ata_ia_status: "PRONTA" } : a
            )
          );
  ...
  const getStatusBadge = (status) => {
    ...
    if (st === "CONCLUIDO")
      return (
        <span className="bg-green-100 text-green-700 text-[10px] px-2 py-0.5 rounded font-bold border border-green-200">
          PRONTO
        </span>
      );
    ...
    if (st.includes("PROCESSANDO") || st.includes("RENDER"))
      return (
        <span className="bg-blue-100 text-blue-700 text-[10px] px-2 py-0.5 rounded font-bold border border-blue-200 animate-pulse">
          PROCESSANDO
        </span>
      );
    ...
    if (st === "PENDENTE")
      return (
        <span className="bg-amber-100 text-amber-700 text-[10px] px-2 py-0.5 rounded font-bold border border-amber-200">
          NA FILA
        </span>
      );
      ...
      <div className="flex h-screen bg-slate-50 font-sans overflow-hidden relative">
        ...
        <div className="w-80 bg-white border-r border-slate-200 flex flex-col z-10 shadow-sm">
          <div className="p-5 border-b border-slate-100">
            ...
            <input
              className="mt-4 w-full bg-slate-50 border p-2 rounded text-sm"
              placeholder="Buscar..."
              value={busca}
                ...
                <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex flex-col gap-1 text-sm text-slate-500 mt-3 bg-slate-50 p-3 rounded-lg border border-slate-100">
                      <div className="flex items-center gap-4">
                        ...
                        <span className="flex items-center gap-1.5 text-slate-700 font-medium">
                          ...
                          {selectedAta.gravacao_inicio
                            ? new Date(selectedAta.gravacao_inicio).toLocaleTimeString([], {
                                hour: "2-digit",
                                minute: "2-digit",
                              })
                            : "--:--"}
                          ...
                          {selectedAta.gravacao_fim
                            ? new Date(selectedAta.gravacao_fim).toLocaleTimeString([], {
                                hour: "2-digit",
                                minute: "2-digit",
                              })
                            : "--:--"}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="p-2 bg-slate-100 rounded hover:text-red-600"
                    >
                      <Trash2 size={18} />
                    </button>
                  </div>
                        ...
                        {String(selectedAta.gravacao_status || "").includes("PROCESSANDO") ? (
                          <>
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400 mt-1 max-w-md mx-auto">
                                O GitHub está trabalhando na compressão. Aguarde...
                              </p>
                            </div>
                          </>
                          ...
                          <>
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400 mt-1 max-w-md mx-auto">
                                Solicitado! O Robô iniciará no próximo ciclo (máx 10min).
                              </p>
                            </div>
                          </>
                        ) : (
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white/60 p-5">
                      <ReactMarkdown
                        ...
                        className="prose prose-slate max-w-none
                          ...
                          prose-p:my-3 prose-li:my-2 prose-ul:my-3 prose-ol:my-3
