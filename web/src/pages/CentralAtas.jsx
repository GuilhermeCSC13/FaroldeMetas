...
function normalizeTitleAndDate(raw, { titulo, dataBR } = {}) {
  ...
  const looksLikeTitleLine =
    safeTitulo && first.toLowerCase().includes(safeTitulo.toLowerCase());
  const startsWithDBO =
    /^d\s*b\s*o\s*[\-–—]/i.test(first) || /^dbo\b/i.test(first);
  ...
  if (!startsWithHeading) {
    if (looksLikeTitleLine || startsWithDBO) {
      ...
      const hasEarlyDate = lines
        .slice(0, Math.min(lines.length, 8))
        .some((l) => /^\s*data\s*:/i.test(l));
...
export default function CentralAtas() {
  ...
  // ✅ NOVO: ref do bloco da ATA para PDF
  const ataPdfRef = useRef(null);
...
`.trim();
  ...
  // ✅ NOVO: Gerar PDF da ATA (somente o bloco da ata)
  ...
  const handleGerarPdfAta = async () => {
    if (!selectedAta) return;
    const el = ataPdfRef.current;
    if (!el) return alert("Não foi possível localizar o conteúdo da ATA para PDF.");

    setGeneratingPdf(true);
    try {
      // força fundo branco e garante layout consistente no canvas
      ...
      const dataBR = selectedAta.data_hora
        ? new Date(selectedAta.data_hora).toLocaleDateString("pt-BR")
        : "";
      const filename = `${safePdfName(selectedAta.titulo)} - ${safePdfName(dataBR)}.pdf`;
  ...
  const handleSolicitarVideo = async () => {
    ...
    try {
      await supabase
        .from("reunioes")
        .update({ gravacao_status: "PENDENTE" })
        .eq("id", selectedAta.id);
  ...
  const handleDeleteMaterial = async (indexToDelete) => {
    ...
    try {
      const novaLista = (selectedAta.materiais || []).filter(
        (_, i) => i !== indexToDelete
      );
      await supabase
        .from("reunioes")
            ...
            <div className="bg-white p-6 rounded shadow-xl border border-red-200">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 p-2 bg-slate-100 rounded"
                >
                  Cancelar
                </button>
                <button
                        ...
                        <span
                          className={`text-[10px] font-black px-2 py-1 rounded-lg uppercase border flex items-center gap-1 w-fit ${
                            iaStatusNorm === "PRONTO" || iaStatusNorm === "PRONTA"
                              ? badgeClass("green")
                              : iaStatusNorm.includes("PROCESSANDO") ||
                                iaStatusNorm.includes("PENDENTE")
                              ? badgeClass("blue")
                              : iaStatusNorm === "ERRO"
                              ? badgeClass("red")
                              : badgeClass("gray")
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex-1">
                      {mediaUrls.audio ? (
                        ...
                        <div className="p-3 bg-slate-50 border rounded text-xs text-slate-400">
                          Sem áudio.
                        </div>
                      )}
                    </div>
                ...
                <div className="mt-2">
                  {isEditing ? (
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white/60 p-5">
                      {/* ✅ NOVO: área capturada pro PDF */}
                      <div ref={ataPdfRef} className="bg-white rounded-lg p-6">
                        <ReactMarkdown
