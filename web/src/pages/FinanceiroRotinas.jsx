...
import { Settings, Target, ClipboardList } from 'lucide-react';
...
const FinanceiroRotinas = () => {
  ...
  const handleSave = async (rotinaId, mesId, valor) => {
    ...
    await supabase.rpc('atualizar_realizado_rotina', {
  ...
  const getCellStatus = (real, meta, tipoComparacao) => {
    ...
    const isGood = (tipoComparacao === '<=' || tipoComparacao === 'menor') ? r <= m : r >= m;
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden font-sans">
      <div className="flex items-center justify-between px-6 py-5 bg-white border-b border-gray-100">
        <h2 className="text-xl font-bold text-gray-800 tracking-tight">Farol de Rotinas — Financeiro</h2>
        <div className="flex items-center gap-4">
          <button onClick={() => window.location.hash = 'metas'} className="px-3 py-1.5 text-sm font-medium text-gray-600 hover:text-blue-600 hover:bg-gray-200 rounded transition-colors">Ir para Metas</button>
          <button onClick={() => setShowConfig(true)} className="p-1.5 text-gray-500 hover:text-blue-600 hover:bg-gray-200 rounded-full transition-colors"><Settings size={18} /></button>
        </div>
      </div>

      <div className="flex-1 overflow-auto">
        {loading ? (
          <div className="flex items-center justify-center h-64 text-gray-400 animate-pulse">Carregando rotinas...</div>
        ) : (
          <table className="w-full text-sm border-separate border-spacing-0">
            <thead className="sticky top-0 z-20 shadow-sm">
              <tr className="bg-[#3b82f6] text-white">
                ...
                {MESES.map(mes => <th key={mes.id} className="p-3 min-w-[100px] text-center font-bold text-xs border-white/10">{mes.label}</th>)}
              </tr>
            </thead>
                ...
                <tr key={row.id} className="group hover:bg-gray-50/80 transition-colors">
                  <td className="sticky left-0 z-10 p-4 bg-white border-r border-gray-100 font-medium text-gray-700 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)]">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-blue-600 font-bold uppercase">{row.responsavel}</span>
                      <span className="truncate" title={row.indicador}>{row.indicador}</span>
                    </div>
                  </td>
                  {MESES.map(mes => {
                    const d = row.meses[mes.id];
                    const bgStatus = getCellStatus(d.realizado, d.meta, row.tipo_comparacao);
                    return (
                      <td key={mes.id} className={`p-0 border-r border-gray-50 relative ${bgStatus}`}>
                        <div className="flex flex-col h-full min-h-[60px] justify-center px-1">
                          <div className="flex items-center justify-center gap-0.5">
                            {row.formato === 'currency' && <span className="text-gray-400 text-[10px]">R$</span>}
                            <input className="w-20 text-center bg-transparent focus:outline-none font-bold text-gray-800" defaultValue={d.realizado} onBlur={(e) => handleSave(row.id, mes.id, e.target.value)} />
                            {row.formato === 'percent' && <span className="text-gray-400 text-[10px]">%</span>}
                          </div>
                          <div className="flex items-center justify-center text-[10px] gap-1 opacity-40">
                            <Target size={8} />
                            <span>{d.meta !== null ? d.meta : '-'}</span>
                          </div>
                        </div>
                      </td>
                    );
...
export default FinanceiroRotinas;
