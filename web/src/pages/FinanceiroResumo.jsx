...
import { AlertTriangle, TrendingUp, CheckCircle, Target, Settings, DollarSign } from 'lucide-react';
...
const FinanceiroResumo = () => {
  ...
  const calculateScore = (meta, realizado, tipo, peso) => {
    ...
    let mult = 0, faixa = 5;
    if (atingimento >= 1.0) { mult = 1.0; faixa = 1; }
    else if (atingimento >= 0.98) { mult = 0.50; faixa = 3; }
    return { score: parseFloat(peso) * mult, faixa };
  };
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Visão Geral — Financeiro</h2>
          <p className="text-sm text-slate-500">Performance consolidada de custos e controladoria</p>
        </div>
        <button onClick={() => setShowConfig(true)} className="flex items-center gap-2 bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-slate-700 transition-colors shadow-sm">
          <Settings size={16} /> Configurar
        </button>
      </div>

      {loading ? <div className="h-64 flex items-center justify-center text-slate-400">Processando dados...</div> : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card KPI title="Score Global" value={metrics.scoreAtual} sub={`/ 100`} Icon={Target} color="blue" />
            <Card KPI title="Metas Batidas" value={metrics.metasBatidas} sub={`/ ${metrics.totalMetas}`} Icon={CheckCircle} color="green" />
            <Card KPI title="Indicadores Críticos" value={metrics.criticos} Icon={AlertTriangle} color="red" />
            <Card KPI title="Tendência" value="Estável" Icon={TrendingUp} color="purple" />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-96">
            <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-sm border border-slate-100">
              <h3 className="font-bold text-slate-700 mb-6">Evolução Score Financeiro</h3>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="score" radius={[4, 4, 0, 0]} barSize={40}>
                    {chartData.map((e, i) => <Cell key={i} fill={e.score >= 90 ? '#22c55e' : '#ef4444'} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
