...
// ID fixo da área Pessoas (Gestão de Pessoas)
const ID_PESSOAS = 8;
...
const PessoasMetas = () => {
  ...
  const fetchAreas = async () => {
    try {
      ...
      if (data && data.length > 0) {
        // Filtra apenas a área de Pessoas (ID = 8)
        const areasFiltradas = data.filter((a) => a.id === ID_PESSOAS);
        if (areasFiltradas.length > 0) {
          setAreas(areasFiltradas);
          setAreaSelecionada(areasFiltradas[0].id);
        } else {
          // fallback: usa a primeira área ativa, se por algum motivo não achar a 8
          setAreas(data);
  ...
  const exportFarol = async (format = "png") => {
    try {
      ...
      a.href = dataUrl;
      a.download = `Farol_Pessoas_2026.${ext}`;
      a.click();
    } catch (e) {
    ...
    <div className="flex flex-col h-full bg-white rounded shadow-sm overflow-hidden font-sans">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Seletor de Área (provavelmente só Pessoas) */}
      ...
      <div className="flex-1 overflow-auto p-4">
        {loading ? (
          ...
          <div ref={tableWrapRef} className="border border-gray-300 rounded overflow-hidden shadow-sm">
                ...
                {metas.map((meta) => (
                  <tr key={meta.id} className="hover:bg-gray-50 text-center">
                    ...
                    {MESES.map((mes) => {
                      ...
                      // ✅ mes=14: input manual, sem alvo e sem score
                      if (mes.id === 14) {
                          ...
                          >
                            <div className="flex flex-col h-full justify-center">
                              <input
                                ...
                                onBlur={(e) => handleSave(meta.id, 14, e.target.value, meta)}
                              />
                            </div>
                          </td>
                        );
                ...
                <tr className="bg-red-600 text-white font-bold border-t-2 border-black">
                  ...
                  {MESES.map((mes) => (
                    <td key={mes.id} className="p-2 text-center border-r border-red-500 text-sm">
                      {mes.id === 14 ? "-" : getTotalScore(mes.id)}
                    </td>
                  ))}
                </tr>
              </tbody>
