...
// IDs fixos das áreas Administrativo (Financeiro + Pessoas)
const IDS_ADMIN = [7, 8];
...
const AdministrativoMetas = () => {
  ...
  // Recarrega as metas sempre que mudar a aba/área
  useEffect(() => {
  ...
  const fetchAreas = async () => {
    try {
      ...
      if (data && data.length > 0) {
        // Filtra apenas Financeiro (7) e Pessoas (8)
        const areasFiltradas = data.filter(a => IDS_ADMIN.includes(a.id));
        if (areasFiltradas.length > 0) {
          setAreas(areasFiltradas);
          setAreaSelecionada(areasFiltradas[0].id);
        } else {
          // fallback se por algum motivo não achar 7/8
          setAreas(data);
