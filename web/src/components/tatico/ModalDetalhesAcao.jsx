...
const ModalDetalhesAcao = ({
  ...
  // ---------------------------------------------------------------------------
  // FUNCÃO DE COLAR NO CAMPO ESPECÍFICO
  // ---------------------------------------------------------------------------
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col relative overflow-hidden">
            ...
            <div className="w-full max-w-sm bg-white border border-red-100 shadow-2xl rounded-2xl p-6 text-center">
              ...
              <div className="space-y-3 text-left">
                <div>
                  ...
                  <input type="text" autoFocus className="w-full border border-slate-300 rounded-lg p-2.5 text-sm outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500" value={delLogin} onChange={e => setDelLogin(e.target.value)} />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-600 uppercase">Senha</label>
                  <input type="password" className="w-full border border-slate-300 rounded-lg p-2.5 text-sm outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500" value={delSenha} onChange={e => setDelSenha(e.target.value)} />
                </div>
              </div>
        ...
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            ...
            <div className="text-sm sm:text-base font-semibold text-gray-800 truncate max-w-xl">{acao.descricao}</div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
             <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="flex flex-col">
                   ...
                   <span className={`text-sm font-semibold ${statusLocal === 'Concluída' ? 'text-green-600' : 'text-amber-600'}`}>{statusLocal}</span>
                </div>
                <div className="flex flex-col">
                   <span className="text-[11px] font-bold text-gray-400 uppercase mb-1">Criação</span>
                   <span className="text-sm text-gray-700 flex items-center gap-1"><Calendar size={14} /> {dataCriacao}</span>
                   {nomeCriador && <span className="text-[10px] text-blue-600 font-medium mt-0.5 flex items-center gap-1"><User size={10} /> Criado por: {nomeCriador}</span>}
                </div>
                <div className="flex flex-col">
                   <span className="text-[11px] font-bold text-gray-400 uppercase mb-1">Vencimento</span>
                   <input type="date" value={vencimento} onChange={(e) => setVencimento(e.target.value)} disabled={inputsDesabilitados} className="border border-gray-300 rounded px-2 py-1 text-sm focus:ring-2 focus:ring-blue-100 disabled:bg-gray-50 w-full" />
                </div>
                <div className="flex flex-col relative">
                   <span className="text-[11px] font-bold text-gray-400 uppercase mb-1">Responsável</span>
                   <input value={responsavelTexto} onChange={(e) => { setResponsavelTexto(e.target.value); setOpenSugestoes(true); }} onFocus={() => setOpenSugestoes(true)} onBlur={() => setTimeout(() => setOpenSugestoes(false), 150)} disabled={inputsDesabilitados || loadingResponsaveis} placeholder="Nome do responsável..." className="border border-gray-300 rounded px-2 py-1 text-sm focus:ring-2 focus:ring-blue-100 disabled:bg-gray-50 w-full" />
                   {openSugestoes && !inputsDesabilitados && (sugestoes || []).length > 0 && (
                    <div className="absolute top-[50px] left-0 w-full bg-white border border-gray-200 rounded shadow-lg z-10 max-h-40 overflow-y-auto">
                      {(sugestoes || []).map(({ u, label }) => (
                        <button key={u.id} type="button" onMouseDown={(ev) => ev.preventDefault()} onClick={() => selecionarResponsavel(u)} className="w-full text-left px-3 py-2 text-xs hover:bg-gray-50 border-b border-gray-100 last:border-0">{label}</button>
                      ))}
                    </div>
                   )}
                </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3 shadow-sm">
              <div>
                ...
                <textarea className="mt-1 w-full border border-gray-300 rounded-lg text-sm p-3 focus:outline-none focus:ring-2 focus:ring-blue-100 disabled:bg-gray-50" rows={3} value={obsAcao} onChange={(e) => setObsAcao(e.target.value)} disabled={inputsDesabilitados} placeholder="Descreva detalhes..." />
              </div>
              ...
              <div>
                ...
                {fotosAcao.length > 0 && (
                  <div className="flex flex-wrap gap-3 mb-3">
                    {fotosAcao.map((url, idx) => <MiniaturaUrl key={`${url}-${idx}`} url={url} />)}
                  </div>
                )}

                {!inputsDesabilitados && (
                  <div className="space-y-2">
                    {/* Botão de Arquivo Tradicional */}
                    <label className="flex flex-col items-center justify-center w-full h-16 border-2 border-dashed border-blue-200 rounded-lg cursor-pointer bg-blue-50/50 hover:bg-blue-50 transition-colors group">
                      <div className="flex flex-row items-center gap-2">
                        ...
                        <p className="text-xs text-gray-500"><span className="font-semibold text-blue-600">Carregar arquivo do PC</span></p>
                      </div>
                      <input type="file" multiple accept="image/*,video/*,application/pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" className="hidden" onChange={(e) => setNovosArquivosAcao(Array.from(e.target.files || []))} />
                    </label>
                    ...
                    {previewsAcao.length > 0 && (
                      <div className="mt-3">
                        ...
                        <div className="flex flex-wrap gap-3">{previewsAcao.map((p) => <MiniaturaArquivo key={p.id} preview={p} />)}</div>
                      </div>
                    )}
                  </div>
                )}
          ...
          <div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-3 shadow-sm relative overflow-hidden">
              ...
              <div>
                ...
                <textarea className="mt-1 w-full border border-gray-300 rounded-lg text-sm p-3 focus:outline-none focus:ring-2 focus:ring-green-100 disabled:bg-gray-50" rows={3} value={resultado} onChange={(e) => setResultado(e.target.value)} disabled={inputsDesabilitados} placeholder="Descreva a solução..." />
              </div>

              <div>
                <span className="text-[11px] font-semibold text-gray-400 uppercase block mb-2">Anexos (Conclusão)</span>
                {fotosConclusao.length > 0 && (
                  <div className="flex flex-wrap gap-3 mb-3">{fotosConclusao.map((url, idx) => <MiniaturaUrl key={`${url}-${idx}`} url={url} />)}</div>
                )}
                  ...
                  <div className="space-y-2">
                    <label className="flex flex-col items-center justify-center w-full h-16 border-2 border-dashed border-green-200 rounded-lg cursor-pointer bg-green-50/50 hover:bg-green-50 transition-colors group">
                      <div className="flex flex-row items-center gap-2">
                        ...
                        <p className="text-xs text-gray-500"><span className="font-semibold text-green-600">Carregar arquivo do PC</span></p>
                      </div>
                      <input type="file" multiple accept="image/*,video/*,application/pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" className="hidden" onChange={(e) => setNovosArquivosConclusao(Array.from(e.target.files || []))} />
                    </label>
                    ...
                    {previewsConclusao.length > 0 && (
                      <div className="mt-3">
                        <div className="text-[10px] font-bold text-gray-400 uppercase mb-2">Prontos para envio:</div>
                        <div className="flex flex-wrap gap-3">{previewsConclusao.map((p) => <MiniaturaArquivo key={p.id} preview={p} />)}</div>
                      </div>
                    )}
                  </div>
                )}
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex flex-col text-[11px] text-gray-500">
             {statusLocal !== "Concluída" && <span className="flex items-center gap-1 text-amber-600 font-medium">! Obrigatório preencher conclusão e anexos para finalizar.</span>}
            <button onClick={() => handleSalvar(null)} disabled={saving || inputsDesabilitados} className="mt-2 self-start px-4 py-2 rounded text-xs font-semibold border border-blue-200 text-blue-700 hover:bg-blue-50 disabled:opacity-50">{saving ? "Salvando..." : "Salvar rascunho"}</button>
          </div>

          <div className="flex items-center gap-2">
            {statusLocal === "Concluída" && (
              <button onClick={handleReabrir} disabled={reabrindo} className="px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100"><RotateCw size={16} /> {reabrindo ? "Reabrindo..." : "Reabrir"}</button>
            )}
            <button onClick={handleExcluirClick} disabled={saving || reabrindo} className="px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 bg-red-50 text-red-700 border border-red-200 hover:bg-red-100"><Trash2 size={16} /> Excluir</button>
            {statusLocal !== "Concluída" && (
              <button onClick={handleFinalizarClick} disabled={!podeConcluirLocal || saving} className={`px-6 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all ${podeConcluirLocal && !saving ? "bg-green-600 text-white hover:bg-green-700 shadow-md transform hover:-translate-y-0.5" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}><CheckCircle size={18} /> Finalizar Ação</button>
            )}
          </div>
        </div>
      </div>
