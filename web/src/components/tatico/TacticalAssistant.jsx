...
const TacticalAssistant = () => {
  ...
  const [messages, setMessages] = useState(() => {
    try {import React, { useState, useRef, useEffect } from 'react';
import { supabase } from '../../supabaseClient';
import { getGeminiFlash } from '../../services/gemini';
import { X, Send, Bot, Loader2, Check, Trash2, CalendarClock, ArrowRight } from 'lucide-react';
import { salvarReuniao } from '../../services/agendaService';
...
const TacticalAssistant = () => {
  ...
  const buscarContextoDados = async () => {
    try {
        ...
        // Busca histórico passado (agora traz MAIS texto da pauta para a IA ler)
        ...
        if (historico?.length) {
            historico.forEach(r => {
                // AQUI ESTÁ O SEGREDO: Diferenciar se tem Pauta ou não
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (cleanText.includes('Ata_Missing')) {
        // Caso: Ata não existe -> Mostra botão
        ...
        // Caso: Resposta normal (Resumo da ata existente)
