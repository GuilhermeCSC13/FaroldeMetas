...
export default function Sidebar() {
  ...
  function toggleSetor(key) {
    setOpenSetores((prev) => ({ ...prev, [key]: !prev[key] }));
  }

  const linkBaseClasses = "flex items-center gap-2 px-4 py-2 text-sm rounded-md transition-colors";
  const linkInactiveClasses = "text-blue-100 hover:text-white hover:bg-blue-600/60";
  ...
  return (
    <aside className="w-64 bg-blue-700 text-white flex flex-col min-h-screen font-sans shrink-0 transition-all duration-300">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto custom-scrollbar">
        <NavLink to="/" className={({ isActive }) => `${linkBaseClasses} ${isActive ? linkActiveClasses : linkInactiveClasses}`} end>
        ...
        <div className="pt-2 pb-1">
          ...
          >
            ...
            {openPlanejamento ? <FaChevronDown className="text-[10px] opacity-70" /> : <FaChevronRight className="text-[10px] opacity-70" />}
          </button>
          ...
          <div className={`overflow-hidden transition-all duration-300 ease-in-out ${openPlanejamento ? 'max-h-[500px] opacity-100 mt-1' : 'max-h-0 opacity-0'}`}>
            <div className="ml-2 pl-2 border-l border-blue-500/30 space-y-1">
              {setores.map((setor) => (
                <div key={setor.key}>
                  <button onClick={() => toggleSetor(setor.key)} className="w-full flex items-center justify-between px-3 py-1.5 text-xs rounded-md hover:bg-blue-600/50 text-blue-50 group">
                    <span className="group-hover:translate-x-1 transition-transform">{setor.label}</span>
                    {openSetores[setor.key] ? <FaChevronDown className="text-[9px] opacity-50" /> : <FaChevronRight className="text-[9px] opacity-50" />}
                  </button>
                  ...
                  {openSetores[setor.key] && (
                    <div className="ml-3 mt-0.5 space-y-0.5 border-l border-blue-500/20 pl-2">
                      {subMenus.map((sub) => {
                        ...
                        return (
                          <button key={sub.key} onClick={() => navigate(`${setor.path}${sub.hash}`)} className={`block w-full text-left text-[11px] px-3 py-1.5 rounded-md transition-colors ${isActive ? "bg-blue-500 text-white font-semibold" : "text-blue-200 hover:text-white hover:bg-blue-600/40"}`}>
                            {sub.label}
                          </button>
                        );
                      })}
        ...
        <NavLink to="/copiloto" className={({ isActive }) => `${linkBaseClasses} ${isActive ? "bg-gradient-to-r from-red-500/20 to-red-600/20 text-red-100 border border-red-500/30 shadow-sm" : "text-red-200 hover:bg-red-500/10 hover:text-white"} mt-4 mb-2`}>
        ...
        <div className="pt-2">
            ...
            <NavLink to="/configuracoes" className={({ isActive }) => `${linkBaseClasses} ${isActive ? linkActiveClasses : linkInactiveClasses}`}>
                <FaCogs className="text-sm" />
                <span>Configurações</span>
            </NavLink>
        </div>
      </nav>
      <div className="px-4 py-3 border-t border-blue-500/40 text-[10px] text-blue-200/60 text-center">Farol Tático v1.2 · 2026</div>
    </aside>
  );
}
